...
let config = {
  ...
  options: {
    smart: {
      ...
      data: ['on', 'off'],
      extra:
        'with smart select turned on linkclump tries to select only the important links',
    },
    ignore: {
      name: 'filter links',
      ...
      extra:
        'filter links that include/exclude these words; separate words with a comma ,',
    },
    copy: {
      name: 'copy format',
      type: 'selection',
    ...
    close: {
      ...
      type: 'textbox',
      extra:
        "number of seconds before closing opened tab (0 means the tab wouldn't close)",
    },
    block: {
    ...
    let undo = $('<a>undo</a>').click(
      ...
      function (event) {
        div_history[event.data.i].replaceWith(
          setup_action(event.data.param, event.data.i)
        );
...
function setup_action(param, id) {
  ...
  setting.append(
    'Activate by ' + config.triggers[param.mouse].name + ' mouse button'
  );
  if (param.key > 0) {
  ...
  list.append(
    "<li>selection box color: <div style='background-color: " +
      param.color +
      "' class='color'></div></li>"
  );
  ...
  for (let opt in config.actions[param.action].options) {
    ...
    if (type === 'checkbox') {
      ...
      if (name === 'ignore') {
        ...
        ignore.unshift(
          (param.options[name] = $('#form_option_selection_' + name).val())
        );
...
$(function () {
  ...
  document
    .getElementById('form_key')
    .addEventListener('change', check_selection);
